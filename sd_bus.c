...
char DAT0_state(void){
return 	PORTDbits.RD0;
}
MASS[4];
//основные регистры, используемые при работе с картой
char 	ocr[4], //информация о вольтаже и поддержке HC
		csr[4], //статус карты, информация о текущем состоянии, предыдущих ошибках
		rca[4], //адрес карты
		cid[15], //идентификационный регистр
		csd[15], //дополнительная информация о карте
		crc_st[1]; //информация о результате завершения записи
...
void load_to_bus(char send,char bus_type) { //передаются байт, который нужно загрузить и на какую линию
		...
		for(k=0;k<BYTE_WIDTH;k++)	{ //поочередно передаем каждый бит
			if(bus_type==CMD_BUS) 
				CMD_low(); //если выбрана линия CMD, сбрасываем значение на линии в 0
			else
				DAT0_low(); //иначе сбрасываем на линии DAT0
			if ((send&0b10000000)!=0x00){ //если старший бит не 0
				if(bus_type==CMD_BUS)
